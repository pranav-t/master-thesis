...
public class NetworkConfig {
	...
	public static String VMS_CLIENT="172.17.0.63";
...
//	public static Integer REGIONSERVERS_NUMOFINSTANCES;
	
	public static List<Node> VIEWMANAGERS;
	public static Integer VIEWMANAGERS_NUMOFINSTANCES;
...
//		    String zookeeper = config.getString("networkconfig.zookeeper");
		    ...
		    NetworkConfig.VMS_CLIENT = config.getString("networkconfig.vmsclient");
		    ...
		    String regionServerNumOfInstances = config.getString("networkconfig.regionServer.numOfInstances");
		    String regionServerMessagePort = config.getString("networkconfig.regionServer.messagePort");
		    String regionServerUpdatePort = config.getString("networkconfig.regionServer.updatePort");
		    
		    List<String> viewManagers = config.getList("networkconfig.viewManager.nodes.node");
		    String viewManagerNumOfInstances = config.getString("networkconfig.viewManager.numOfInstances");
...
//		    NetworkConfig.REGIONSERVERS_NUMOFINSTANCES = Integer.parseInt(regionServerNumOfInstances);
		    for (String string : regionServers) {
		    	NetworkConfig.REGIONSERVERS.add(new Node(string, Integer.parseInt(regionServerMessagePort), Integer.parseInt(regionServerUpdatePort),  new SSHConnection(string, 22, SystemConfig.SSH_USERNAME, SystemConfig.SSH_PASSWORD)));			
			}
		    NetworkConfig.VIEWMANAGERS_NUMOFINSTANCES = Integer.parseInt(viewManagerNumOfInstances);
...
//		    
//		    log.info(NetworkConfig.class, "NetworkConfig.VMS_CLIENT:"+NetworkConfig.VMS_CLIENT);
//		    
...
//		log.info(HbaseClient.class, "NetworkConfig.ZOOKEEPER_QUORUM: "+NetworkConfig.ZOOKEEPER_QUORUM);
		
		if(NetworkConfig.ZOOKEEPER_CLIENTPORT != 0){
			conf.set("hbase.zookeeper.property.clientPort", NetworkConfig.ZOOKEEPER_CLIENTPORT+"");
//			log.info(HbaseClient.class, "NetworkConfig.ZOOKEEPER_CLIENTPORT: "+NetworkConfig.ZOOKEEPER_CLIENTPORT);
		}
	
		conf.set("hbase.master", NetworkConfig.HBASE_MASTER.getIpAddress());
//		log.info(HbaseClient.class, "NetworkConfig.HBASE_MASTER: "+NetworkConfig.HBASE_MASTER.getIpAddress());

		if(NetworkConfig.HBASE_MASTER.getMessagePort() != 0){
			conf.set("hbase.master.port", NetworkConfig.HBASE_MASTER.getMessagePort()+"");
//			log.info(HbaseClient.class, "NetworkConfig.HBASE_MASTER_PORT: "+NetworkConfig.HBASE_MASTER.getMessagePort());
		}
