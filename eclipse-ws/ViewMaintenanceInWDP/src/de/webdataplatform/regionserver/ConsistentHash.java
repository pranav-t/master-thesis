...
// private final Map<T, Integer> replicas = new HashMap<T, Integer>();
 ...
 public void add(T node) {
	 ...
	 for (int i = 100; i < numberOfReplicas+100; i++) {
	 ...
	 synchronized(this){ 
		 ...
		 while(!placeFound){
			 ...
			 if(circle.get(hashFunction.hash(node.toString()+i+c))!= null){
//				 System.out.println("Oh mein Gott");
				 c=c+111;
			 }else{
			
				 circle.put(hashFunction.hash(node.toString()+i+c), node);
				 placeFound=true;
			 }	 
	 	}
	 }
