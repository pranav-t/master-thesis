...
public class CopyUpdateClient extends Thread {
...
//	  int port = 3144;
	...
	public CopyUpdateClient(Log log, SystemID rsNetworkAddress, ViewManager viewManager, Queue<String> updateQueue, AtomicLong updatesSent) {
...
//	    this.port = viewManager.getPort();
	  ...
	  public void run() {  //run the service
		  ...
		  try{
		    ...
		    log.update(this.getClass(),"starting handshake for "+viewManager.getVMName()+":"+viewManager.getSystemID().getUpdatePort());
		    
			Message message = new Message(rsNetworkAddress, Command.REGIONSERVER_START_UPDATE_PROCESSING, rsNetworkAddress.toString());
...
//			log.info(this.getClass(),"message "+message);
		    ...
		    log.update(this.getClass(), "receiving handshake from "+viewManager.getVMName()+":  "+empfangeneNachricht);
//		    if(empfangeneNachricht.equals("ok"));
		    ...
		    while(running){
		    	...
		    	if(update != null){
		    		...
		    		log.info(this.getClass(), "Polled new update for:"+viewManager.getVMName()+", update:"+update);
