...
//			table.setAutoFlushTo(true);
	...
	public HTable getTable(String tableName) {
		...
		log.info(this.getClass(), "table-cache: "+tableCache);
...
//			s = Bytes.toInt(result.getValue(Bytes.toBytes("colfam1"), Bytes.toBytes("aggValue")));
	...
	public Result get(byte[] tableName, byte[] key, List<byte[]> columnFamilies, List<byte[]> columns) {
		
		return getWithTimestampRange(tableName, key, columnFamilies, columns, null, null, null);
	}
	...
	public void put(byte[] viewTableName,  Put put) {
		

		if(lastRowKey != null && lastRowKey.equals(Bytes.toString(put.getRow()))){
			try {
				log.info(this.getClass(), "same row-key, waiting:");
				Thread.sleep(1);
...
//			while(start + 800000 >= System.nanoTime());
		}
...
//				
//				
//				log.info(this.getClass(), "table_name: "+Bytes.toString(viewTableName));
//				log.info(this.getClass(), "table-instance: "+getTable(Bytes.toString(viewTableName)));
				
			getTable(Bytes.toString(viewTableName)).put(put);
				
//				Get get = new Get(put.getRow());
				
//				exists = getTable(Bytes.toString(viewTableName)).exists(get);
//				log.info(this.getClass(), "exists: "+exists);
//				if(x==100)break;
//				x++;
//			}
//			getTable(Bytes.toString(viewTableName)).flushCommits();
			long stopTime = new Date().getTime();
			
			if((stopTime - startTime) > SystemConfig.VIEWMANAGER_CRUDOPERATIONSALARMTHRESHOLD){
				log.info(this.getClass(), "put very slow: "+(stopTime - startTime)+" ms:  tableName="+viewTableName);
			}
...
//		log.update(this.getClass(), "putting to table:"+viewTableName+",  key="+key+", checkQualifier="+checkQualifier+", checkValue="+checkValue+", columns="+MatchingRow.mapToString(columns));
	...
	public void delete(byte[] viewTableName, Delete delete){ 
		...
		try {
			...
			getTable(Bytes.toString(viewTableName)).delete(delete);

			lastRowKey=Bytes.toString(delete.getRow());
