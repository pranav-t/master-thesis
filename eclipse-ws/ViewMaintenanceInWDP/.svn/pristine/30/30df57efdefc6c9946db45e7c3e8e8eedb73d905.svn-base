...
import de.webdataplatform.view.operation.Join;
...
//	static Map<String, List<String>> viewIndex = new HashMap<String, List<String>>();
	...
	public void check(String baseTableName, String viewTableName, String viewDefinition) {
		...
		try {
			...
			ViewOperation viewMode = ViewOperation.valueOf(viewDefinition.split(",")[0]);
...
//			
	...
	private boolean compareViews(String viewTableName, String viewDefinition, Map<String, List<String>> calculatedView, Configuration conf) throws IOException {
		...
		for (String key : calculatedView.keySet()) {
			...
			if(viewMode.equals(ViewOperation.AGGREGATION_SUM) || viewMode.equals(ViewOperation.AGGREGATION_COUNT) || viewMode.equals(ViewOperation.AGGREGATION_MIN) || viewMode.equals(ViewOperation.AGGREGATION_MAX)){
...
//			if(createViewTable instanceof CreateAggregationView){
			...
			if(viewMode.equals(ViewOperation.SELECTION)){
...
//				System.out.println("calculatedValue:"+calculatedValue);
			...
			if(viewMode.equals(ViewOperation.INDEX)){
			...
			if(viewMode.equals(ViewOperation.JOIN)){
...
//							System.out.println("aggValue:"+Bytes.toString(aggValue.getValue()));
	...
	//						System.out.println("values: "+values);
							if(values == null){
								...
								if(viewMode.equals(ViewOperation.AGGREGATION_MIN))values.add(String.valueOf(Long.MAX_VALUE));
								else values.add("0");
							}
							...
							if(viewMode.equals(ViewOperation.AGGREGATION_SUM))value += Long.parseLong(Bytes.toString(aggValue.getValue()));
							if(viewMode.equals(ViewOperation.AGGREGATION_COUNT))value += 1;
							if(viewMode.equals(ViewOperation.AGGREGATION_MIN)){
							...
							if(viewMode.equals(ViewOperation.AGGREGATION_MAX))if(Long.parseLong(Bytes.toString(aggValue.getValue())) > value)value = Long.parseLong(Bytes.toString(aggValue.getValue()));
