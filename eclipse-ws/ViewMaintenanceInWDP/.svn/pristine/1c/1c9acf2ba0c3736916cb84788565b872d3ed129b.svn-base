...
//		System.out.println(sshConnection);
	...
	public static List<String> sendCommand(Log log, SSHConnection sshConnection, List<String> commands) {
		...
		return sendCommand(log, sshConnection, commands, true);
	}
	
	
	public static List<String> sendCommand(Log log, SSHConnection sshConnection, List<String> commands, boolean executeBackground) {
		...
		try {
			...
			if(executeBackground)command ="nohup sh -c \"";
            ...
            if(executeBackground)command += "\" > /dev/null 2>&1 &";
