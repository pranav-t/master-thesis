...
//		fs.mkdirs(path);
		...
		SystemID systemID = new SystemID(Component.regionServer, "rs1", "deltaserver1", 3343,4433);
		
		AtomicLong updatesRetrieved = new AtomicLong();
		
		
		Queue incomingUpdates = new ConcurrentLinkedQueue<BaseTableOperation>();
		
//		ViewDefinitions getViews = new ViewDefinitions(log, "viewdefinitions");
//		getViews.loadViewDefinitions();
		
		WALReader walReader = new WALReader(log, systemID.getName(), systemID.getIp(), incomingUpdates);
